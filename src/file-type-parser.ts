...
import { CheckOptions, _check } from "./check";
import { stringToBytes } from "./string-to-bytes";
import * as Token from "token-types";
import * as strtok3 from "strtok3";
import { uint32SyncSafeToken } from "./uint32-sync-safe-token";
import { tarHeaderChecksumMatches } from "./tar-header-checksum-matches";
...
export class FileTypeParser {
	...
	private check(header: number[], options?: CheckOptions): boolean {
		return _check(this.buffer, header, options);
	}

	private checkString(header: string, options?: CheckOptions): boolean {
		return this.check(stringToBytes(header), options);
	}

	async parse(tokenizer: ITokenizer): Promise<FileType | void> {
	...
	async readTiffTag(bigEndian: boolean): Promise<FileType | void> {
	...
	async readTiffIFD(bigEndian: boolean): Promise<FileType | void> {
	...
	async readTiffHeader(bigEndian: boolean): Promise<FileType | void> {
